...
<div id="content">
	<?php
	$sql = "SELECT * FROM Tournoi WHERE ID IN (Select ID_Tournoi FROM Inscriptions WHERE ID_User = (SELECT ID FROM Users WHERE Username = '".strtoupper($_SESSION["username"])."')) AND DateFin > NOW() - INTERVAL 1 DAY";
	$stmt = $db->prepare($sql);
	$stmt->execute();
